...
function ExperienceTimeline({ items }: { items: ExperienceItem[] }) {
  return (
    <div className="mt-5 space-y-6 border-l border-zinc-200 pl-6 dark:border-zinc-800">
      {items.map((item) => (
        <div key={`${item.role}-${item.org}`} className="relative">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-zinc-500 dark:text-zinc-400">{item.org}</span>
          </div>
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
      ...
      <h3 className="mt-10 text-sm font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
      ...
      <h3 className="mt-12 text-sm font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
